...
class _CreatorWalletScreenState extends State<CreatorWalletScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() => _error = tr('تعذر تحميل بيانات المحفظة', 'Unable to load wallet data'));
      }
    } finally {
            ...
            : RefreshIndicator(
                ...
                child: ListView(padding: const EdgeInsets.all(16), children: [
                  ...
                  ...history.map((item) {
                    ...
                    final isPayout = row['direction']?.toString() == 'out';
                    return Card(
                        child: ListTile(
                            title: Text(isPayout
                                ? tr('دفعة مرسلة', 'Payout sent')
                                : row['title']?.toString() ??
                                tr('دخل', 'Income')),
                            subtitle: Text(row['status']?.toString() ?? ''),
                            trailing: Text(
                                '${isPayout ? '-' : '+'}${_money(row['amount'])}',
                                style: TextStyle(
                                    color: isPayout ? colors.error : colors.success,
                                    fontWeight: FontWeight.bold))));
                  }),
                ]),
              ),
