...
export default function PaymentsTable() {
    ...
    const [canRejectPayments, setCanRejectPayments] = useState(false);
    ...
    const fetchPayments = async () => {
        try {
            ...
            const isSuperadmin = payload?.superadmin === true;
            setCanConfirmPayments(isSuperadmin || role === 'admin' || role === 'moderator');
            setCanRejectPayments(isSuperadmin);
    ...
    const handleDelete = async (paymentId: string) => {
        if (!window.confirm('هل تريد حذف هذا الدفع نهائياً؟')) {
            return;
        }
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(data?.error || 'فشل حذف الدفع');
            }
                            ...
                            <tr key={payment.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="py-4 px-4 text-center">
                                    {payment.status === 'pending' ? (
                                        canConfirmPayments || canRejectPayments ? (
