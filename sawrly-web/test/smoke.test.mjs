...
test('creator wallet derives balance from payments and recorded payouts', () => {
  const walletRoutePath = path.join(cwd, 'app', 'api', 'wallet', 'route.ts');
  const walletRoute = fs.readFileSync(walletRoutePath, 'utf8');
  assert.ok(walletRoute.includes("requireRole(req, ['creator'])"));
  assert.ok(walletRoute.includes('wallet_payouts'));
  assert.ok(walletRoute.includes("'earning'::text AS entry_type"));
  assert.ok(walletRoute.includes("'payout'::text AS entry_type"));

  const payoutRoutePath = path.join(cwd, 'app', 'api', 'admin', 'wallet', 'payouts', 'route.ts');
  const payoutRoute = fs.readFileSync(payoutRoutePath, 'utf8');
  assert.ok(payoutRoute.includes('Payout exceeds the creator available balance'));
  assert.ok(payoutRoute.includes('creator_payout_recorded'));

  const walletSchemaPath = path.join(cwd, 'lib', 'wallet-schema.ts');
  assert.ok(fs.readFileSync(walletSchemaPath, 'utf8').includes('CREATE TABLE IF NOT EXISTS wallet_payouts'));
});
