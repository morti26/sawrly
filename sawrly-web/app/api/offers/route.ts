...
import { getUserFromRequest, requireActiveCreator } from '@/lib/auth';
...
export async function GET(req: NextRequest) {
    ...
    let sql = `
      SELECT o.id, o.title, o.description, o.price_iqd, o.image_url, o.creator_id,
             ...
             CASE
                 ...
                 ELSE EXISTS(
                     SELECT 1
                     FROM offer_likes olm
                     WHERE olm.offer_id = o.id
                       AND olm.user_id = $1::uuid
                 )
             END AS liked_by_me,
    ...
    const params: any[] = [currentUser?.userId ?? null];
    let paramIdx = 2;
    ...
    if (savedOnly) {
        sql += ` AND EXISTS (
            SELECT 1
            FROM offer_likes ols
            WHERE ols.offer_id = o.id
              AND ols.user_id = $1::uuid
        )`;
    }
...
export async function POST(req: NextRequest) {
    ...
    const client = await getClient();
    try {
        const creatorOfferCountRes = await client.query(
            `
        ...
        const creatorOfferCount = Number(creatorOfferCountRes.rows[0]?.total ?? 0);
        if (creatorOfferCount >= 2) {
            return NextResponse.json(
                { error: 'يمكنك نشر عرضين فقط كحد أقصى.' },
                { status: 403 }
            );
        }
