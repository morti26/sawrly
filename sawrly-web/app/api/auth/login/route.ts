...
import { ADMIN_PANEL_ROLES, ensureCreatorNotFrozen, isSuperAdminUser, verifyPassword, signToken } from '@/lib/auth';
import {
...
export async function POST(req: NextRequest) {
    try {
        ...
        const rateLimitKey = buildLoginRateLimitKey(req, normalizedEmail);
        ...
        const frozen = await ensureCreatorNotFrozen({ userId: user.id, role: user.role });
        if (frozen) {
            return NextResponse.json({
                error: frozen.error,
        ...
        const token = signToken({
            ...
            email: user.email,
            role: user.role,
            superadmin: isSuperAdminUser({ userId: user.id, email: user.email, role: user.role }),
        });
