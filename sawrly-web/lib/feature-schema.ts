...
let userProfileSchemaEnsuring: Promise<void> | null = null;
...
async function isUserProfileSchemaReady(): Promise<boolean> {
    try {
        await query(`
            SELECT bio, gender, avatar_url, cover_image_url, country, city, subscription_plan, subscription_expires_at
            FROM users
            LIMIT 1
        `);
        return true;
...
async function ensureUserProfileSchemaInternal(): Promise<void> {
    ...
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS cover_image_url TEXT`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS country VARCHAR(120)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS city VARCHAR(120)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_plan VARCHAR(20)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_expires_at TIMESTAMP WITH TIME ZONE`);
}
...
export async function ensureUserProfileSchema(): Promise<void> {
    if (userProfileSchemaEnsured) return;
    if (!userProfileSchemaEnsuring) {
        userProfileSchemaEnsuring = (async () => {
            if (await isUserProfileSchemaReady()) {
                userProfileSchemaEnsured = true;
                return;
            }

            try {
                await ensureUserProfileSchemaInternal();
            } catch (error: any) {
                if (!isInsufficientPrivilegeError(error)) {
                    throw error;
                }
            ...
            .finally(() => {
                userProfileSchemaEnsuring = null;
            });
    }
    await userProfileSchemaEnsuring;
}
