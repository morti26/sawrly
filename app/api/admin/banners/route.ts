...
function normalizeBannerRow<T extends Record<string, unknown>>(row: T): T {
    const mediaItems = Array.isArray(row.media_items)
        ? row.media_items.map((item) => ({
            ...item,
            url: typeof item?.url === 'string' ? normalizeUploadUrl(item.url) : item?.url,
        }))
        : row.media_items;

    return {
        ...row,
        image_url: typeof row.image_url === 'string' ? normalizeUploadUrl(row.image_url) : row.image_url,
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json(res.rows.map(normalizeBannerRow));
    } catch (e) {
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json(normalizeBannerRow(res.rows[0]));
    } catch (e) {
