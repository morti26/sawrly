...
const MAX_FREE_VIDEO_DURATION_SECONDS = 60;
...
async function getCreatorMediaPlan(userId: string): Promise<'free' | 'monthly' | 'unlimited'> {
    ...
    if (!row) return 'free';
    const plan = String(row.subscription_plan ?? '').trim().toLowerCase();
    ...
    if (plan === 'yearly' || plan === 'plus' || plan === 'monthly_plus') {
        return 'unlimited';
    }
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const mediaPlan = await getCreatorMediaPlan(auth.user!.userId);
        if (mediaPlan !== 'unlimited') {
            ...
            const videoCount = Number(countRes.rows[0]?.total ?? 0);
            const maxVideos = mediaPlan == 'monthly' ? 8 : MAX_FREE_CREATOR_VIDEOS;
            if (videoCount >= maxVideos) {
                return NextResponse.json(
                    {
                        error:
                            mediaPlan === 'monthly'
                                ? 'الخطة الشهرية المحدودة تسمح بحد أقصى 8 فيديوهات.'
                                : 'يلزم اشتراك لرفع أكثر من 4 فيديوهات.',
                    },
                    { status: 403 }
                );
            }
