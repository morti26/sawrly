...
import { initializeFirebase, sendFCMNotification } from '@/lib/firebase-admin';
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (nextPaymentStatus === 'confirmed' && payment.quote_creator_id && payment.quote_client_id) {
            setImmediate(async () => {
                try {
                    await initializeFirebase();

                    // Get creator and client FCM tokens
                    const creatorTokensRes = await query(
                        `
                        SELECT device_token FROM fcm_device_tokens
                        WHERE user_id = $1
                        ORDER BY last_used_at DESC
                        `,
                        [payment.quote_creator_id]
                    );

                    const clientTokensRes = await query(
                        `
                        SELECT device_token FROM fcm_device_tokens
                        WHERE user_id = $1
                        ORDER BY last_used_at DESC
                        `,
                        [payment.quote_client_id]
                    );

                    // Send to creator
                    for (const row of creatorTokensRes.rows) {
                        await sendFCMNotification(
                            row.device_token,
                            'Payment Confirmed! 🎉',
                            'A client has paid for your offer. Check your projects to get started.',
                            {
                                paymentId: payment.id,
                                projectId: projectId || '',
                                type: 'payment_confirmed_creator',
                            }
                        );
                    }

                    // Send to client
                    for (const row of clientTokensRes.rows) {
                        await sendFCMNotification(
                            row.device_token,
                            'Payment Successful! ✅',
                            'Your payment has been confirmed. The creator will start working on your project.',
                            {
                                paymentId: payment.id,
                                projectId: projectId || '',
                    ...
                    await logOpsError({
                        source: 'api.payments.webhook.fcm',
                        level: 'warn',
